...
class FavouriteArtistsAdapter(private val listener: FavouriteArtistsListener) :
    ...
    private var artistsList = mutableListOf<FavouriteArtist>()
    ...
    private fun isPositionFooter(position: Int): Boolean {
        return position == artistsList.size
    }

    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        if (holder is ArtistViewHolder) {
            holder.bind(artistsList[position])
            holder.itemView.setOnClickListener { listener.onFavouriteArtistClicked(artistsList[position]) }
        } else {
            holder.itemView.setOnClickListener { listener.onFavouriteArtistsMoreClicked() }
        }
    }

    fun updateList(list: List<FavouriteArtist>) {
    ...
    class ArtistViewHolder(
        ...
        fun bind(item: FavouriteArtist) {
            with(binding) {
                tvAlbumName.text = item.artist
